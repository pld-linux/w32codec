#
# Conditional build:
# _with_licence_agreement	- generates package
#
...
Name:		w32codec
Version:	0.90pre7
Release:	5%{?_with_licence_agreement:wla}
Group:		Libraries
License:	Free for non-commercial use
%{?_with_licence_agreement:Source0:	http://www.mplayerhq.hu/MPlayer/releases/%{name}-%{version}.tar.bz2}
%{?_with_licence_agreement:Source1:	http://www1.mplayerhq.hu/MPlayer/releases/codecs/qt6dlls.tar.bz2}
%{?_with_licence_agreement:Source2:	http://www1.mplayerhq.hu/MPlayer/releases/codecs/qtextras.tar.bz2}
%{?_with_licence_agreement:Source3:	http://www1.mplayerhq.hu/MPlayer/releases/codecs/rp8codecs.tar.bz2}
%{?_with_licence_agreement:Source4:	http://www1.mplayerhq.hu/MPlayer/releases/codecs/rp9codecs.tar.bz2}
%{?_with_licence_agreement:Source5:	http://www1.mplayerhq.hu/MPlayer/releases/codecs/xanimdlls.tar.bz2}
%{?_with_licence_agreement:Source6:     http://www1.mplayerhq.hu/MPlayer/releases/codecs/dmocodecs.tar.bz2}
%{?_with_licence_agreement:Source7:     http://www1.mplayerhq.hu/MPlayer/releases/codecs/mjpeg2kdlls.tar.bz2}
Autoreqprov:	false
ExclusiveArch:	%{ix86}
%{?!_with_licence_agreement:Requires:	wget}
...
files.
%if %{?!_with_licence_agreement:1}%{?_with_licence_agreement:0}
...
w32codec.install --with licence_agreement %{w32codecDIR}/%{name}-%{version}-%{release}.src.rpm
%endif
...
%prep
%{?_with_licence_agreement:%setup -q -n %{name}-0.90}
...
%{?_with_licence_agreement:install_codecs}

%install
rm -rf $RPM_BUILD_ROOT
install -d $RPM_BUILD_ROOT%{_libdir}/win32

%if %{?_with_licence_agreement:0}%{?!_with_licence_agreement:1}
install -d $RPM_BUILD_ROOT%{_bindir}
cat <<EOF >$RPM_BUILD_ROOT%{_bindir}/w32codec.install
#!/bin/sh
if [ "\$1" = "--with" -a "\$2" = "licence_agreement" ]
then
	...
	( cd \$SPECDIR
	%{_bindir}/builder -nc -ncs --with licence_agreement --opts --target=%{_target_cpu} w32codec.spec
...
\$0 --with licence_agreement %{w32codecDIR}/%{name}-%{version}-%{release}.src.rpm
"
fi
EOF
%else
...
%{?_with_licence_agreement:%{_libdir}/win32}
%{?!_with_licence_agreement:%attr(755,root,root) %{_bindir}/w32codec.install}
